...
def get_pyzed_directory():
    try:
        ...
        lines = subprocess.check_output(
            call_list, encoding="utf-8", errors="replace").splitlines()
...
parser = argparse.ArgumentParser(
    description='Helper script to download and setup the ZED Python API')
...
if sys.platform == "win32":
    ...
    print("Unknown system.platform: %s  Installation failed, see setup.py." %
          sys.platform)
    sys.exit(1)
...
whl_python_version = "-cp" + \
    str(PYTHON_MAJOR) + str(PYTHON_MINOR) + "-cp" + \
    str(PYTHON_MAJOR) + str(PYTHON_MINOR)
...
whl_file_URL = base_URL + str(ZED_SDK_MAJOR) + "." + \
    str(ZED_SDK_MINOR) + "/whl/" + OS_VERSION + "/" + whl_file
whl_file = os.path.join(dirname, whl_file)
...
if check_valid_file(whl_file):
    ...
    if sys.platform == "win32":
        ...
        py_vers = int(int(PYTHON_MAJOR) * math.pow(10,
                      len(PYTHON_MINOR)) + int(PYTHON_MINOR))
        ...
        for file in files:
            if os.path.isfile(source_dir + file):
                ...
                print("ERROR : An error occurred, 'pip' failed to copy dll file " +
                      source_dir + file + " (pyzed was NOT correctly setup)")
